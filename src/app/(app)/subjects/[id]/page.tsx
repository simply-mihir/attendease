...
export default function SubjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const scheduledExam = currentOrUpcomingExam
    ? subjectReminders.find((r: any) => r.category === "exam" && r.title === currentOrUpcomingExam.name)
    : null;
  ...
  const availableSlots: { id: string; label: string; time: string; weight?: number }[] = [];
  if (subject) {
  ...
  return (
    ...
    <PageTransition direction="right" staggerChildren={false} className="max-w-4xl mx-auto space-y-6 pb-16">
      ...
      {currentOrUpcomingExam && (
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h3 className="font-black text-base text-text">Course Exam</h3>
          ...
          {scheduledExam ? (
            <div className="flex items-center justify-between bg-[#06d6a0]/10 border border-[#06d6a0]/40 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-text">Scheduled for {new Date(scheduledExam.dueDate).toLocaleDateString()}</p>
                  <p className="text-xs font-bold text-[#06d6a0]">Time: {scheduledExam.dueTime}</p>
                </div>
              </div>
              <button
                onClick={() => deleteSubjectReminder(scheduledExam.id)}
                className="btn-3d-coral px-3 py-1.5 text-xs font-bold flex items-center gap-1.5"
              >
      ...
      >
        ...
        {subjectReminders.filter((r: any) => r.category !== "exam").length === 0 ? (
          <p className="text-text-muted text-sm py-6 text-center font-bold">No tasks or generic reminders for this subject yet.</p>
        ) : (
          <div className="space-y-2.5">
            {subjectReminders.filter((r: any) => r.category !== "exam").map((rem: any) => (
              <div
                key={rem.id}
                className={clsx(
      ...
      {showEditSubjectModal && (
        <div className="fixed inset-0 md:pl-[var(--sidebar-width)] z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 md:pl-[var(--sidebar-width)] z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="rounded-2xl p-6 sm:p-7 max-w-md w-full border-2 border-gray-200 bg-white shadow-[0_12px_0_0_#d1d5db] dark:border-[#2a2a3d] dark:bg-[#141425] dark:shadow-[0_12px_0_0_#0d0d1a] space-y-4">
            <div className="flex items-center gap-3">
