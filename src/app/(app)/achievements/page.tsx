      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {categories.map(cat => (
          <button
            ...
            className={`rounded-xl border-2 px-4 py-2 text-sm font-bold transition-all duration-150 cursor-pointer flex items-center gap-1
          ...
          >
            {cat.label} 
            {isLoading ? (
              <span className="inline-block ml-1 opacity-60"><FieldLoader size="sm" /></span>
            ) : (
              <span>({cat.count})</span>
            )}
          </button>
        ...
        {filteredBadges.map((badge) => {
          ...
          return (
            ...
            >
              ...
              {earned && isLatestStreak && !isLoading && (
                <>
