...
- If the image is not a timetable, return: {"subjects": [], "error": "This doesn't appear to be a timetable"}`;
  ...
  try {
    ...
    // If the AI flagged it as not a timetable
    if (parsed.error) {
      return NextResponse.json(
    ...
    const subjects = parsed.subjects.map(
      (s: Record<string, unknown>, i: number) => ({
        name: (typeof s.name === "string" ? s.name : "Unknown Subject").trim(),
        code:
          typeof s.code === "string" ? s.code.trim() || null : null,
        instructor:
          typeof s.instructor === "string"
            ? s.instructor.trim() || null
            : null,
        ...
        schedules: (Array.isArray(s.schedules) ? s.schedules : [])
          .map((sch: Record<string, unknown>) => ({
            ...
            dayOfWeek:
              DAY_MAP[
                typeof sch.day === "string" ? sch.day.toLowerCase() : ""
              ] ?? -1,
            startTime:
              typeof sch.startTime === "string" ? sch.startTime : "09:00",
            endTime:
              typeof sch.endTime === "string" ? sch.endTime : "10:00",
            room:
              typeof sch.room === "string"
                ? sch.room.trim() || null
                : null,
          }))
          .filter(
            (sch: { dayOfWeek: number }) => sch.dayOfWeek >= 0
          ),
      })
    );
    ...
    return NextResponse.json({
      ...
      totalSlots: subjects.reduce(
        (a: number, s: { schedules: unknown[] }) =>
          a + s.schedules.length,
        0
      ),
    });
  } catch (err) {
